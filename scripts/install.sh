...
#   gemini-cli   -- Install extension to ~/.gemini/extensions/my-claude-agents/
#   opencode     -- Copy agents to .opencode/agents/ in current directory
#   cursor       -- Copy rules to .cursor/rules/ in current directory
#   aider        -- Copy CONVENTIONS.md to current directory
#   windsurf     -- Copy .windsurfrules to current directory
#   openclaw     -- Copy workspaces to ~/.openclaw/my-claude-agents/
#   qwen         -- Copy SubAgents to .qwen/agents/ (project-scoped)
...
#   --parallel        Run install for each selected tool in parallel
...
box_row() {
  local raw="$1"
  local visible; visible="$(strip_ansi "$raw")"
...
# Agent directories — covers all agent locations in this repo
AGENT_DIRS=(
  agents
  business-growth
  c-level
...
interactive_select() {
  ...
  while true; do
    printf "\n"
    box_top
    box_row "${C_BOLD}  my-claude-agents -- Tool Installer${C_RESET}"
    box_bot
    printf "\n"
    ...
    for t in "${ALL_TOOLS[@]}"; do
      ...
      local dot; [[ "${detected_map[$i]}" == "1" ]] && dot="${C_GREEN}[*]${C_RESET}" || dot="${C_DIM}[ ]${C_RESET}"
      local chk; [[ "${selected[$i]}" == "1" ]] && chk="${C_GREEN}[x]${C_RESET}" || chk="${C_DIM}[ ]${C_RESET}"
      printf "  %s  %s)  %s  %s\n" "$chk" "$num" "$dot" "$label"
      (( i++ )) || true
    done
    ...
    case "$input" in
      ...
      "")
        local any=false
        local s; for s in "${selected[@]}"; do [[ "$s" == "1" ]] && any=true && break; done
        if $any; then break
        else printf "  ${C_YELLOW}Nothing selected.${C_RESET}\n"; sleep 1; fi ;;
      *)
        local toggled=false num
        for num in $input; do
          if [[ "$num" =~ ^[0-9]+$ ]]; then
            local idx=$(( num - 1 ))
    ...
    local l; for (( l=0; l<lines; l++ )); do printf '\033[1A\033[2K'; done
  done
...
install_gemini_cli() {
  ...
  local dest="${HOME}/.gemini/extensions/my-claude-agents"
  local count=0
  [[ -d "$src" ]] || { err "integrations/gemini-cli missing. Run convert.sh first."; return 1; }
  mkdir -p "$dest/skills"
  cp "$src/gemini-extension.json" "$dest/gemini-extension.json" 2>/dev/null || true
  local d
  while IFS= read -r -d '' d; do
    local name; name="$(basename "$d")"
    ...
    (( count++ )) || true
  done < <(find "$src/skills" -mindepth 1 -maxdepth 1 -type d -print0 2>/dev/null)
...
install_openclaw() {
  ...
  local dest="${HOME}/.openclaw/my-claude-agents"
  local count=0
  ...
  while IFS= read -r -d '' d; do
    ...
    mkdir -p "$dest/$name"
    cp "$d/SOUL.md" "$d/AGENTS.md" "$d/IDENTITY.md" "$dest/$name/"
    (( count++ )) || true
  done < <(find "$src" -mindepth 1 -maxdepth 1 -type d -print0)
...
install_qwen() {
  ...
  warn "Qwen Code: project-scoped. Run '/agents manage' in Qwen Code to refresh."
}
...
install_kimi() {
  ...
  ok "Kimi Code: $count agents -> $dest"
}
...
main() {
  ...
  if   [[ "$interactive_mode" == "yes" ]]; then use_interactive=true
  elif [[ "$interactive_mode" == "auto" && -t 0 && -t 1 && "$tool" == "all" ]]; then use_interactive=true; fi
  ...
  if $use_interactive; then
    ...
    header "my-claude-agents -- Scanning for installed tools..."
    printf "\n"
    local t
    for t in "${ALL_TOOLS[@]}"; do
      if is_detected "$t" 2>/dev/null; then
  ...
  printf "\n"
  header "my-claude-agents -- Installing agents"
  printf "  Repo:       %s\n  Installing: %s\n" "$REPO_ROOT" "${SELECTED_TOOLS[*]}"

  local installed=0 t i=0
  local n_selected=${#SELECTED_TOOLS[@]}

  if $use_parallel; then
    local install_out_dir; install_out_dir="$(mktemp -d)"
    ...
    for t in "${SELECTED_TOOLS[@]}"; do [[ -f "$install_out_dir/$t" ]] && cat "$install_out_dir/$t"; done
