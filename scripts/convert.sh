#!/usr/bin/env bash
#
# convert.sh — Convert agent .md files into tool-specific formats.
#
# Reads all agent files from the agent directories and outputs
...
# Resolve color field — map human color names to #RRGGBB; default gray if unrecognized.
resolve_opencode_color() {
  ...
  case "$c" in
    ...
    sonnet|haiku|opus) mapped="#6366F1" ;;  # model values — map to indigo
    *)              mapped="$c" ;;
  esac
...
convert_opencode() {
  ...
  name="$(get_field "name" "$file")"
  description="$(get_field "description" "$file")"
  # Fall back: try 'color' field, then 'domain' field mapped to a color, then default
  local raw_color
  raw_color="$(get_field "color" "$file")"
  [[ -z "$raw_color" ]] && raw_color="$(get_field "domain" "$file")"
...
# Aider and Windsurf accumulate into temp files then write at the end.
AIDER_TMP="$(mktemp)"
WINDSURF_TMP="$(mktemp)"
trap 'rm -f "$AIDER_TMP" "$WINDSURF_TMP"' EXIT

cat > "$AIDER_TMP" <<'HEREDOC'
# my-claude-agents — AI Agent Conventions
#
# This file provides Aider with the full roster of specialized AI agents.
...
# my-claude-agents — AI Agent Rules for Windsurf
#
...
main() {
  ...
  header "my-claude-agents — Converting agents to tool-specific formats"
  echo "  Repo:   $REPO_ROOT"
  echo "  Output: $OUT_DIR"
  echo "  Tool:   $tool"
  echo "  Date:   $TODAY"
  ...
  if $use_parallel && [[ "$tool" == "all" ]]; then
    local parallel_tools=(antigravity gemini-cli opencode cursor openclaw qwen kimi)
    local parallel_out_dir
    parallel_out_dir="$(mktemp -d)"
    info "Converting: ${#parallel_tools[@]}/${n_tools} tools in parallel..."
