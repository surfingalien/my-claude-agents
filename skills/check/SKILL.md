...
- V1: all Finnhub response handlers have null guards → grep `\.price` for unguarded access
- V2: all portfolio writes are in transactions → grep `db.query` outside `BEGIN/COMMIT`
- V3: JWT token expiry uses `<=` not `<` → grep auth middleware
- V4: all Claude API calls have error boundaries → grep `anthropic.messages.create` for try/catch
