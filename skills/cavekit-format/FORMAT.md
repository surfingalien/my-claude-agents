...
name: cavekit-format
description: >-
  SPEC.md schema and caveman encoding reference for cavekit. Defines the six
  fixed sections (§G §C §I §V §T §B), pipe table rules, addressing syntax
  (§V.2, §T.n), caveman symbol table (→ ∀ ∃ ! ⊥ ≤ ≥ ∈ ∉), and the one-file
  rule. Load this when writing or reading any SPEC.md.
