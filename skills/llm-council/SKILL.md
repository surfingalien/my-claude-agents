...
description: >-
  ...
  independently analyze it from different angles, peer-review each other
  anonymously, and synthesize a final verdict. Anti-sycophancy design — built
  to tell you what you don't want to hear. Produces an HTML visual report +
  markdown transcript. Based on Karpathy's LLM Council methodology.
  TRIGGER on: 'council this', 'run the council', 'war room this',
  'pressure-test this', 'stress-test this', 'debate this'.
  ALSO TRIGGER on genuine decisions with stakes: 'should I X or Y',
  'which option', 'is this the right move', 'I can\'t decide', 'I\'m torn
  between', 'validate this', 'get multiple perspectives'.
  DO NOT trigger on: factual lookups, simple yes/no, pure creation tasks
  (write me a tweet), summaries, or casual should-I without real tradeoffs.
origin: tenfoldmarc/llm-council-skill
owner: surfingalien
license: MIT
---
...
The council is particularly valuable for FinSurfing (React + Vite + Express + PostgreSQL + Anthropic API on Railway) decisions where Claude's natural agreeableness is a liability:

**Good council questions for FinSurfing:**
- "Should I build the options analytics feature or focus on portfolio attribution first?"
- "Freemium with usage caps vs. flat $29/month subscription — which model fits?"
- "Should I pivot FinSurfing from retail traders to RIAs/advisors?"
- "Is the Monte Carlo retirement module the right differentiator or is it feature bloat?"
- "Which of these 3 landing page angles is strongest: AI-powered / portfolio intelligence / trading research?"
- "Should I hire a part-time developer or build the next phase solo with Claude Code?"

**Outsider advisor is especially valuable here** — the curse of knowledge is severe in fintech. What's obvious to a developer building trading tools is completely opaque to the retail investor who would actually pay for it.

**The Executor advisor maps directly to your philosophy** — Ship > Perfect, Automate Ruthlessly. If a strategic idea has no clear Monday-morning first step, the Executor will say so.

## Related Skills

- `make-plan` — Use after the council verdict to plan execution of the recommendation
- `do` — Orchestrate the council's "one thing to do first" as a multi-step build
- `design-taste-frontend` — If the council recommends a landing page change, use this to implement it
- `redesign-existing-projects` — If the verdict calls for a UI overhaul
