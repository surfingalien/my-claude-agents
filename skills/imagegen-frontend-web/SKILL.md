...
description: >-
  Elite frontend image-direction skill. Generates one separate horizontal
  image per section — never compressed into one board. Enforces composition
  variety, background-image freedom, varied CTAs, hero scale selection,
  narrative concept spine, and a single consistent palette across all images.
  TRIGGER when: user asks for website design references, section comps,
  landing page mockups, or any frontend visual references before coding.
origin: taste-skill
owner: surfingalien
---
...
## FinSurfing Image Generation

For FinSurfing marketing site section images:

**Hero (Section 1)**:
- Hero Scale: Giant Statement
- Background Mode: Full-bleed dark atmosphere with tonal overlay
- Composition: Bottom-left text over background (stock chart / data visualization as atmosphere)
- Palette: Deep dark `#090E1A`, cyan accent `#06B6D4`, single strong CTA
- Copy: Short, direct — "Stock Intelligence, Powered by AI" (not "Revolutionize Your Trading")

**Features (Section 2)**:
- Background Mode: Solid dark surface with inline assets
- Composition: Asymmetric bento — 3 feature tiles + 1 wide tile
- No pill badges. No "00 Section" labels.

**Social Proof (Section 3)**:
- Background Mode: Slightly lighter surface variant
- Composition: Clean metric strip or split quote wall
- Real-looking data: "47.2% average return identification", not "99.99% accuracy"

**CTA (Section 4)**:
- Background Mode: Single strong accent field or atmospheric image
- Composition: Centered low, single CTA button, one trust cue line
- Mini minimalist hero energy — confident restraint

## Related Skills

- `image-to-code` — Full pipeline skill: generate → analyze → implement
- `design-taste-frontend` — Implementation rules after image generation
- `imagegen-frontend-mobile` — Mobile screen image generation companion
- `brandkit` — Brand-kit board generation (logo systems, identity decks)