...
description: >-
  ...
  become bolder, loud designs that should become quieter, live browser
  iteration on UI elements, or ambitious visual effects. Not for backend-only
  or non-UI tasks. Use /impeccable <command> [target].
argument-hint: "[craft|shape|audit|polish|critique|harden|animate|colorize|typeset|layout|delight|overdrive|clarify|adapt|optimize|distill|bolder|quieter|onboard|init|document|extract|live] [target]"
user-invocable: true
allowed-tools:
  - Bash(npx impeccable *)
...
When applying this skill to FinSurfing (React + Vite + Express + PostgreSQL + Anthropic API on Railway):

- **Register**: `product` for the trading dashboard/app; `brand` for the marketing/landing page
- **DESIGN.md anchor**: FinSurfing palette — deep dark `oklch(7% 0.005 230)`, cyan accent `oklch(65% 0.15 200)`, desaturated gain green `oklch(55% 0.12 145)`, loss red `oklch(50% 0.14 25)`
- **Typography**: Geist + Geist Mono (matches the trading terminal aesthetic)
- **AI analysis panels**: Never use glassmorphism or gradient text. Use `product` register — earned familiarity, not brand distinctiveness
- **Stock data tables**: `divide-y` with monospace numbers, not card-per-row. `font-variant-numeric: tabular-nums` on all financial figures
- **Key commands for FinSurfing**: `/impeccable audit` on any new UI surface before shipping; `/impeccable harden` on any form or user-input flow; `/impeccable polish` before each Railway deploy

## Related Skills

- `design-taste-frontend` — Complementary anti-slop rules; use impeccable for craft depth, taste-skill for layout variance enforcement
- `redesign-existing-projects` — Use before `/impeccable` on existing FinSurfing pages
- `design-is` — Dieter Rams audit (different lens, use both)
- `full-output-enforcement` — Pair to prevent truncated component output
- `stitch-design-taste` — Use `/impeccable document` output as input to Stitch DESIGN.md generation
