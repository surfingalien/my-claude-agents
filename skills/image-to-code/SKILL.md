...
name: image-to-code
description: >-
  Image-first website design pipeline: generate reference images first, analyze
  them deeply, then implement the frontend to match as closely as possible.
  Bans lazy under-generation, cards-inside-cards UI, and hero clutter.
  TRIGGER when: visual quality is the primary concern, user asks for a
  beautiful/premium/creative website, or any multi-section landing page build
  where you have image generation available.
origin: taste-skill
owner: surfingalien
---
...
For FinSurfing marketing/landing pages using this skill:

**Workflow**:
1. Generate 1 hero image — dark OLED, stock data atmosphere, single CTA, no clutter
2. Generate 1 features bento image — 3-4 asymmetric tiles: Intelligence, AI Analysis, Portfolio
3. Generate 1 social proof / testimonial image — clean quote wall or metric strip
4. Generate 1 CTA/footer image — decisive, single action, trust cues
5. Deep-analyze all 4, extract exact colors/typography/spacing
6. Implement in React + Vite + Tailwind, faithful to the references

**Anti-drift enforcement**: The implemented FinSurfing landing page must still feel like the dark premium platform in the reference images — not a generic SaaS landing page with a blue primary button.

## Related Skills

- `design-taste-frontend` — Full anti-slop rules for implementing after analysis
- `imagegen-frontend-web` — Section-by-section image generation skill (pairs naturally)
- `soft-skill` — Premium component architecture for the implementation phase
- `full-output-enforcement` — Ensure complete component output after analysis
- `gpt-taste` — Stricter variant with GSAP if motion is needed