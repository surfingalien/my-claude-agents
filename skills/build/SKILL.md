...
**Typical build invocations for FinSurfing**:
```
/ck:build --next          → pick lowest pending §T task
/ck:build §T.3            → build specific task
/ck:build --all           → execute all pending tasks in order
```

**Verification commands for FinSurfing**:
- Frontend: `npm run test` or `npm run build`
- Backend: `npm test` in Express API directory
- Full stack: `npm run dev` + manual API check

**Backprop on failure**: if a Finnhub API null-check fails during build, cavekit auto-invokes `/ck:spec bug:` to generate §B entry + §V invariant before retrying — the bug class can never recur silently.

## Related Skills

- `cavekit-spec` → create/amend the spec this skill builds against
- `cavekit-check` → verify no drift after build completes
- `do` → multi-subagent orchestration; cavekit-build for spec-anchored single-thread execution
- `caveman` → compression mode active during build output
