...
description: >-
  Elite mobile app image-generation skill. Premium, app-native screen concepts
  and flows for iOS, Android, and cross-platform. Prioritizes readable text,
  multi-screen consistency, controlled color palettes, non-generic creative
  direction, textured surfaces, image-led composition, and clean phone mockup
  framing. Generates images only — does not write code.
  TRIGGER when: user asks for mobile app screen mockups, onboarding flows,
  app concept images, or mobile UI references.
origin: taste-skill
owner: surfingalien
---
...
## FinSurfing Mobile Context

For a FinSurfing mobile app concept:

**Platform Mode**: iOS-native premium

**Key Screens**:
1. **Home/Dashboard** — Stock watchlist with clean hierarchy. P&L in desaturated green/red. Muted palette, no chart spam.
2. **Stock Detail** — AI analysis card as the hero element. Claude's output styled as a premium editorial block, not a chat bubble.
3. **AI Chat/Analysis** — Conversational AI interface with financial context. Think Linear-style chat, not generic ChatGPT.
4. **Portfolio** — Clean metric tiles. Gainers/losers with controlled color. No fake dashboards.

**Design Bible for FinSurfing Mobile**:
- Platform: iOS-native premium
- Palette: Deep dark base (`#090E1A`) + cyan accent (`#06B6D4`) + desaturated P&L colors
- Typography: Clean system-like sans + monospace for data
- Texture: Subtle micro-noise on card surfaces
- Mockup: Clean iPhone Pro frame, visible border, content-first

## Related Skills

- `imagegen-frontend-web` — Web section image generation companion
- `image-to-code` — Full pipeline: generate mobile refs → analyze → implement
- `design-taste-frontend` — Implementation rules after mobile image analysis