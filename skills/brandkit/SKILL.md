...
name: brandkit
description: >-
  Premium brand-kit image generation. Creates high-end brand-guidelines boards,
  logo systems, identity decks, and visual-world presentations. Trained for
  minimalist, cinematic, editorial, dark-tech, luxury, cultural, security,
  gaming, developer-tool, and consumer-app brand systems.
  TRIGGER when: user asks for brand identity, logo concepts, brand guidelines,
  identity deck, or visual brand system images.
origin: taste-skill
owner: surfingalien
---
...
## FinSurfing Brand Context

For FinSurfing brand-kit generation:

**Brand Strategy**:
- Category: Consumer financial intelligence / AI trading research
- Audience: Retail investors, active traders, quantitative hobbyists
- Personality: Sharp, intelligent, confident — "the Bloomberg terminal that doesn't need a manual"
- Core Metaphor: Surfing a wave of data — fluid, forward momentum, in control
- Logo Idea: Abstract wave + cursor/signal fusion — motion + precision

**Visual Mode**: Dark Developer / Builder
- Near-black panels, monospace accents, subtle grid
- Cyan as the primary accent (matches surfing/water theme + tech precision)
- Logo: Minimal wave-form mark that works as icon, wordmark, UI mark, and pattern

**Default Panel Set** (3×2):
1. Logo Cover — wordmark + mark on deep dark background, massive negative space
2. Logo Construction — wave + signal geometry breakdown
3. Digital Application — browser chrome, dashboard fragment showing stock card
4. Brand Essence — tagline: "Surf the signal." Large typography, sparse
5. Color System — dark base + cyan swatch + gain green + loss red + neutral scale
6. System Detail — UI chips, data badge system, monospace snippet

**Surfingalien Identity Note**: The `surfingalien` handle (github.com/surfingalien/my-claude-agents) and `dev@surfingalien.ai` suggest the builder identity. The brand should feel like a solo developer's premium product — not a VC-funded startup with a generic wordmark.

## Related Skills

- `imagegen-frontend-web` — Web section image generation (use after brand kit is defined)
- `imagegen-frontend-mobile` — Mobile screen generation with brand applied
- `stitch-design-taste` — Generate DESIGN.md that encodes the brand kit as design tokens
- `soft-skill` — Implement the brand in premium component architecture