...
description: >-
  ...
  Reads the brief, infers the right design direction, and ships interfaces that
  do not look templated. Real design systems when applicable, audit-first on
  redesigns, strict 14-point pre-flight check.
  TRIGGER when: any frontend build, landing page, portfolio, or redesign task.
  The default frontend skill for all UI work.
origin: taste-skill
owner: surfingalien
---
...
When applying this skill to FinSurfing (React + Vite + Express):

**Brief Read**: *"Reading this as: consumer financial intelligence platform for retail investors and traders, with a premium dark-tech language, leaning toward Ethereal Glass vibe + Tailwind v4 + Motion."*

**Dial defaults for FinSurfing**:
- `DESIGN_VARIANCE: 7` — confident asymmetry, not chaotic
- `MOTION_INTENSITY: 6` — purposeful motion on data updates and AI analysis reveals
- `VISUAL_DENSITY: 6` — enough density to feel like a real tool, enough breathing room to feel premium

**Key patterns**:
- Hero: Cinematic Center, dark OLED background, stock chart animation behind text, single CTA
- Feature bento: 3 features (Intelligence, AI Analysis, Portfolio) + 1 wide social proof
- AI analysis panels: Double-Bezel nested architecture (see `soft-skill`)
- Data tables: `divide-y` on monospace type — never generic bordered cards per row
- P&L colors: desaturated gain green `#10B981` and loss red `#EF4444` — never neon
- Loading states: Skeleton loaders matching analysis card shape — no spinners

**Pre-flight check**: Run the full 14-point checklist. Specifically for FinSurfing:
- Zero em-dashes in any copy
- Button contrast on dark backgrounds (white text on cyan CTA must be WCAG AA)
- Hero fits viewport on 13" MacBook (the primary target device for traders)
- No "Unleash the power of AI" or similar generic financial copy

## Related Skills

- `soft-skill` — Premium agency-level component architecture
- `gpt-taste` — Stricter variant with GSAP enforcement
- `redesign-existing-projects` — Upgrade existing FinSurfing UI
- `minimalist-ui` — When the brief calls for restraint
- `design-is` — Audit the design after building
- `full-output-enforcement` — Pair to ensure complete component output
- `image-to-code` — Image-first pipeline for visual-quality-critical sections