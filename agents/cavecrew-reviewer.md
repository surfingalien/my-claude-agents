...
Use before every Railway deploy: `cavecrew-reviewer` on the diff catches null-check gaps, missing PostgreSQL transactions, React hook dependency issues, and off-by-one errors in financial calculations. Runs on Haiku — fast review loop.
